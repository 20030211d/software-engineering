<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="派发日期" prop="paifariqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.paifariqi" 
                type="date"
                :readonly="ro.paifariqi"
                placeholder="派发日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.paifariqi" label="派发日期" prop="paifariqi">
              <el-input v-model="ruleForm.paifariqi" 
                placeholder="派发日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.paifariqi = this.getCurDate()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yajin = true;
          ...
          if(o=='paifariqi'){
            this.ruleForm.paifariqi = obj[o];
	    this.ro.paifariqi = true;
            continue;
          }
        }
      }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaoyuankapaifa/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaoyuankapaifaCrossAddOrUpdateFlag = false;
						 this.parent.search();
