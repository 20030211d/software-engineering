<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="校门" prop="xiaomen">
          <el-input v-model="ruleForm.xiaomen" 
              placeholder="校门" clearable  :readonly="ro.xiaomen"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	xiaomen : false,
      ...
      rules: {
          xiaomen: [
                { required: true, message: '校门不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaomen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaomenCrossAddOrUpdateFlag = false;
						 this.parent.search();
