<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="校园卡号" prop="xiaoyuankahao">
          <el-select :disabled="ro.xiaoyuankahao" @change="xiaoyuankahaoChange" v-model="ruleForm.xiaoyuankahao" placeholder="请选择校园卡号">
            <el-option
                v-for="(item,index) in xiaoyuankahaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.xiaoyuankahao" label="校园卡号" prop="xiaoyuankahao">
              <el-input v-model="ruleForm.xiaoyuankahao" 
                placeholder="校园卡号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="医疗记录" prop="yiliaojilu">
          <el-input v-model="ruleForm.yiliaojilu" 
              placeholder="医疗记录" clearable  :readonly="ro.yiliaojilu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="医疗记录" prop="yiliaojilu">
              <el-input v-model="ruleForm.yiliaojilu" 
                placeholder="医疗记录" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='xiaoyuankahao'){
            this.ruleForm.xiaoyuankahao = obj[o];
	    this.ro.xiaoyuankahao = true;
            continue;
          }
          if(o=='xueshengxuehao'){
	    ...
	    this.ro.xueshengxingming = true;
          ...
          if(o=='yiliaojilu'){
            this.ruleForm.yiliaojilu = obj[o];
	    this.ro.yiliaojilu = true;
            continue;
          }
          if(o=='yiliaofeiyong'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/xiaoyuankapaifa/xiaoyuankahao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xiaoyuankahaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    xiaoyuankahaoChange () {
      this.$http({
        url: `follow/xiaoyuankapaifa/xiaoyuankahao?columnValue=`+ this.ruleForm.xiaoyuankahao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.xueshengxuehao){
                this.ruleForm.xueshengxuehao = data.data.xueshengxuehao
              }
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yiliaojilu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yiliaojiluCrossAddOrUpdateFlag = false;
						 this.parent.search();
